 ...
 */
static void *DynaHashAlloc(size_t size);
...
static int	choose_nelem_alloc(size_t entrysize);
...
add_size(size_t s1, size_t s2)
{
	...
	if (result < s1 || result < s2)
		warnx("requested shared memory size overflows size_t");
	return result;
}
 ...
 */
static int
string_compare(const char *key1, const char *key2, size_t keysize)
{
...
hash_create(const char *tabname, long nelem, HASHCTL *info, int flags)
{
	...
	if (!init_htab(hashp, nelem))
		warnx("failed to initialize hash table \"%s\"", hashp->tabname);
 ...
 */
static int
choose_nelem_alloc(size_t entrysize)
{
	int			nelem_alloc;
	size_t		elementSize;
	size_t		allocSize;
 ...
 */
size_t
hash_estimate_size(long num_entries, size_t entrysize)
{
	size_t		size;
	long		nBuckets,
				nSegments,
...
calc_bucket(HASHHDR *hctl, uint32_t hash_val)
{
	uint32_t		bucket;
							...
							bool *foundPtr)
{
	...
	switch (action)
	{
		...
		case HASH_ENTER:
			...
			if (hashp->frozen)
				warnx("cannot insert into frozen hashtable \"%s\"",
					 hashp->tabname);
	...
	warnx("unrecognized hash action code: %d", (int) action);
...
{
	if (hashp->isshared)
		warnx("cannot freeze shared hashtable \"%s\"", hashp->tabname);
	if (!hashp->frozen && has_seq_scans(hashp))
		warnx("cannot freeze hashtable \"%s\" because it has active scans",
			 hashp->tabname);
	hashp->frozen = true;
}
...
expand_table(HTAB *hashp)
{
	 ...
	 */
	if ((uint32_t) new_bucket > hctl->high_mask)
	{
		hctl->low_mask = hctl->high_mask;
		hctl->high_mask = (uint32_t) new_bucket | hctl->low_mask;
	}
...
dir_realloc(HTAB *hashp)
{
	...
	CurrentDynaHashCxt = hashp->hcxt;
#endif
	p = (HASHSEGMENT *) hashp->alloc((size_t) new_dirsize);
...
hash_corrupted(HTAB *hashp)
{
	 ...
	 */
	if (hashp->isshared)
		warnx("hash table \"%s\" corrupted", hashp->tabname);
	else
		warnx("hash table \"%s\" corrupted", hashp->tabname);
}
...
register_seq_scan(HTAB *hashp)
{
	if (num_seq_scans >= MAX_SEQ_SCANS)
		warnx("too many active hash_seq_search scans, cannot start one on \"%s\"",
			 hashp->tabname);
...
deregister_seq_scan(HTAB *hashp)
{
	...
	warnx("no hash_seq_search scan for hash table \"%s\"",
		 hashp->tabname);
}
...
AtEOXact_HashTables(bool isCommit)
{
	...
	if (isCommit)
	{
		...
		for (i = 0; i < num_seq_scans; i++)
		{
			warnx("leaked hash_seq_search scan for hash table %p",
				 seq_scan_tables[i]);
		}
	}
	num_seq_scans = 0;
