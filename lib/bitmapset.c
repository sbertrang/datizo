...
static const uint8_t rightmost_one_pos[256] = {
...
static const uint8_t number_of_ones[256] = {
...
bms_make_singleton(int x)
{
	...
	if (x < 0)
		warnx("negative bitmapset member not allowed");
	wordnum = WORDNUM(x);
	bitnum = BITNUM(x);
	...
	for (wordnum = 0; wordnum < nwords; wordnum++)
	{
		...
		if (w != 0)
		{
			if (result >= 0 || HAS_MULTIPLE_ONES(w))
				warnx("bitmapset has multiple members");
...
bms_hash_value(const Bitmapset *a)
{
	...
	if (lastword < 0)
		return 0;				/* All empty sets hash to 0 */
	return hash_any((const unsigned char *) a->words,
