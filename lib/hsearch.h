 ...
 * used directly.)
 */
typedef void *(*HashCopyFunc) (void *dest, const void *src, size_t keysize);
 ...
 */
typedef void *(*HashAllocFunc) (size_t request);
...
{
	HTAB	   *hashp;
	uint32_t		curBucket;		/* index of current bucket */
...
extern void *hash_search(HTAB *hashp, const void *keyPtr, HASHACTION action,
			bool *foundPtr);
extern uint32_t get_hash_value(HTAB *hashp, const void *keyPtr);
extern void *hash_search_with_hash_value(HTAB *hashp, const void *keyPtr,
							uint32_t hashvalue, HASHACTION action,
							bool *foundPtr);
...
extern size_t hash_estimate_size(long num_entries, size_t entrysize);
extern long hash_select_dirsize(long num_entries);
extern size_t hash_get_shared_size(HASHCTL *info, int flags);
 ...
 */
extern uint32_t string_hash(const void *key, size_t keysize);
extern uint32_t tag_hash(const void *key, size_t keysize);
extern uint32_t oid_hash(const void *key, size_t keysize);
extern uint32_t bitmap_hash(const void *key, size_t keysize);
extern int	bitmap_match(const void *key1, const void *key2, size_t keysize);
